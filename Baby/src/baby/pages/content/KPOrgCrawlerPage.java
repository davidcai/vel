 ...
 *
 */
public final class KPOrgCrawlerPage extends BabyPage
{
	public final static String COMMAND = BabyPage.COMMAND_CONTENT + "/kporg-crawler";
	...
	public String getTitle() throws Exception
	{
		...
		{
			return getString("content:KPOrgCrawler.Title");
		}
	}
	...
	private void renderRegionTree() throws Exception
	{
		if (isParameter("crawl"))
		{
			...
			writeEncode(getString("content:KPOrgCrawler.CrawlInitiated", new Date()));
			write("</div>");
		}
		else
		{
			...
			{
				write("<div class=InfoMessage>");
				writeEncode(getString("content:KPOrgCrawler.LastCrawl", lastCrawl));
				write("</div>");
			}
		}
		...
		new LinkToolbarControl(this)
			.addLink(	getString("content:KPOrgCrawler.InitiateCrawl"), getPageURL(getContext().getCommand(), new ParameterMap("crawl", "")), "icons/standard/lady-bug-16.png")
			.render();
		...
		if (regions.size()==0)
		{
			writeEncode(getString("content:KPOrgCrawler.NoArticles"));
			return;
		}
...
//				column("").width(1); // checkbox
				column(getString("content:KPOrgCrawler.ArticleTitle"));
				column(getString("content:KPOrgCrawler.SubSection"));
			}
...
//				writeCheckbox("chk_" + article.getID().toString(), null, false);
				...
				cell();
				writeLink(article.getTitle(), getPageURL(EditArticlePage.COMMAND, new ParameterMap(EditArticlePage.PARAM_ID, article.getID())));
