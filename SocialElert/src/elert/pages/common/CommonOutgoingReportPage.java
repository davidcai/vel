...
public class CommonOutgoingReportPage extends CommonTimeReportPage
{	
	...
	protected void renderGraph(Date from, Date to, UUID regionID, UUID serviceAreaID, UUID facilityID, UUID schedulerID, UUID physicianID, UUID procedureID) throws Exception
	{
		...
		QueryIterator<ChannelStat> iter = ElertStore.getInstance().queryChannelStats(	false,
																						from,
																						to,
																						regionID,
																						serviceAreaID,
		...
		try
		{
			...
			{
				writeEncode(getString("elert:OutgoingReport.NoResults"));
				return;
			}
	...
	{
		return getString("elert:OutgoingReport.Help");
	}

	@Override
	public String getTitle() throws Exception
	{
		return getString("elert:OutgoingReport.Title");
	}
}
