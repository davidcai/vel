...
public class ChosenNotif extends ElertPage
{
	public final static String COMMAND = ElertPage.COMMAND_PATIENT + "/chosen.notif";
	...
	{
		return getString("patient:ChosenNotif.Title", Setup.getAppTitle(getLocale()), Setup.getAppOwner(getLocale()), this.opening.getDateTime());
	}

	@Override
	public void renderHTML() throws Exception
	{
		writeEncode(getString("patient:ChosenNotif.LongText", this.opening.getDateTime(), getProceduresString(), this.facility.getName()));
	}

	@Override
	public void renderShortText() throws Exception
	{
		write(getString("patient:ChosenNotif.ShortText", this.opening.getDateTime(), getProceduresString(), this.facility.getName()));
	}
	...
	{
		write(getString("patient:ChosenNotif.LongText", this.opening.getDateTime(), getProceduresString(), this.facility.getName()));
	}
	...
	{		
		...
		for (int i=0; i<procIDs.size(); i++)
		{
			...
			if (i>0)
			{
				if (i==procIDs.size()-1)
				{
					result.append(getString("patient:ChosenNotif.And"));
				}
				else
				{
					result.append(getString("patient:ChosenNotif.Comma"));
				}
			}
